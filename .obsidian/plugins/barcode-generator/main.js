...
var require_Barcode = __commonJS({
  "node_modules/jsbarcode/bin/barcodes/Barcode.js"(exports) {
    ...
    var Barcode2 = function Barcode3(data, options) {
      _classCallCheck(this, Barcode3);
      this.data = data;
      this.text = options.text || data;
      this.options = options;
    };
    exports.default = Barcode2;
  }
});
...
var require_JsBarcode = __commonJS({
  "node_modules/jsbarcode/bin/JsBarcode.js"(exports, module2) {
    "use strict";
    ...
    var JsBarcode3 = function JsBarcode4(element, text, options) {
      var api = new API();
    ...
    JsBarcode3.getModule = function(name2) {
    ...
    if (typeof jQuery !== "undefined") {
      jQuery.fn.JsBarcode = function(content, options) {
        ...
        return JsBarcode3(elementArray, content, options);
      };
    }
    module2.exports = JsBarcode3;
  }
});
...
var BarcodeSettingsTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  set_barcode_name(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Barcode name tag").setDesc("Codeblock tag that will be used to determine if the code block is meant to be a barcode.").addText((text) => text.setPlaceholder("barcode").setValue(this.plugin.settings.barcode_name).onChange(async (value) => {
      this.plugin.settings.barcode_name = value;
      await this.plugin.saveSettings();
    }));
  }
  set_format(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Default format").setDesc("Select the default barcode format (it will not affect pre-formatted or custom barcodes).").addDropdown((compenent) => compenent.addOptions(BARCODE_SETTINGS_NAME).setValue(this.plugin.settings.defaultFormat).onChange(async (value) => {
      this.plugin.settings.defaultFormat = value;
      await this.plugin.saveSettings();
    }));
  }
  set_background(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Background color").addColorPicker((color) => color.setValue(this.plugin.settings.background).onChange(async (value) => {
      this.plugin.settings.background = value;
      await this.plugin.saveSettings();
    }));
  }
  set_line(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Line color").addColorPicker((color) => color.setValue(this.plugin.settings.lineColor).onChange(async (value) => {
      this.plugin.settings.lineColor = value;
      await this.plugin.saveSettings();
    }));
  }
  set_text_display(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Display text").addToggle((component) => component.setValue(this.plugin.settings.displayValue).onChange(async (value) => {
      this.plugin.settings.displayValue = value;
      await this.plugin.saveSettings();
    }));
  }
  set_font(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Font style").addText((text) => text.setPlaceholder("monospace").setValue(this.plugin.settings.font).onChange(async (value) => {
      this.plugin.settings.font = value;
      await this.plugin.saveSettings();
    }));
  }
  set_font_size(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Font size").setDesc("Size of the font in pixels.").addText((text) => text.setPlaceholder("font size").setValue("" + this.plugin.settings.fontSize).onChange(async (value) => {
      this.plugin.settings.fontSize = +value;
      await this.plugin.saveSettings();
    }));
  }
  set_text_align(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Text align").setDesc("Horizontal position of the text in the barcode.").addDropdown((compenent) => compenent.addOption("left", "Left").addOption("center", "Center").addOption("right", "Right").setValue(this.plugin.settings.textAlign).onChange(async (value) => {
      this.plugin.settings.textAlign = value;
      await this.plugin.saveSettings();
    }));
  }
  set_text_position(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Text position").setDesc("Vertical position of the text in the barcode.").addDropdown((compenent) => compenent.addOption("top", "Top").addOption("bottom", "Bottom").setValue(this.plugin.settings.textPosition).onChange(async (value) => {
      this.plugin.settings.textPosition = value;
      await this.plugin.saveSettings();
    }));
  }
  set_width(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Barcode width").addSlider((componenet) => componenet.setLimits(1, 5, 1).setValue(this.plugin.settings.width).onChange(async (value) => {
      this.plugin.settings.width = value;
      await this.plugin.saveSettings();
    }));
  }
  set_height(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Barcode height").addSlider((componenet) => componenet.setLimits(25, 500, 1).setValue(this.plugin.settings.height).onChange(async (value) => {
      this.plugin.settings.height = value;
      await this.plugin.saveSettings();
    }));
  }
  set_margin(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Margin").addSlider((componenet) => componenet.setLimits(1, 20, 1).setValue(this.plugin.settings.margin).onChange(async (value) => {
      this.plugin.settings.margin = value;
      await this.plugin.saveSettings();
    }));
  }
  set_text_margin(containerEl) {
    new import_obsidian.Setting(containerEl).setName("Text margin").addSlider((componenet) => componenet.setLimits(1, 20, 1).setValue(this.plugin.settings.textMargin).onChange(async (value) => {
...
var SimpleBarcodeProcessor = class {
  constructor(app) {
    this.barcode_app = app;
  }
  processBarcode(settings, tag_name) {
    import_obsidian2.MarkdownPreviewRenderer.unregisterPostProcessor(
      this.barcode_app.registerMarkdownCodeBlockProcessor(tag_name, (content, el, _) => {
        const canvas = document.createElement("canvas");
        (0, import_jsbarcode.default)(canvas, content.trim(), {
...
var import_jsbarcode2 = __toESM(require_JsBarcode());
var AdvancedBarcodeProcessor = class {
  constructor(app) {
    this.barcode_app = app;
  }
  processBarcode(settings, tag_name) {
    this.barcode_app.registerMarkdownCodeBlockProcessor(tag_name, (content, el, _) => {
      const canvas = document.createElement("canvas");
      (0, import_jsbarcode2.default)(canvas, content.trim(), {
...
// src/main.ts
var Barcode = class extends import_obsidian3.Plugin {
